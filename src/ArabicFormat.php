...
final class ArabicFormat
{
    ...
    /** تطبيع رقم الهوية للتخزين/المقارنة (غربي + بدون مسافات). */
    public static function normalizeNationalId(string|int|float|null $value): string
    {
        $id = self::toWesternDigits(trim((string) ($value ?? '')));
        return preg_replace('/\s+/u', '', $id) ?? $id;
    }

    /**
     * تعبير SQL يحوّل الأرقام العربية/الفارسية في عمود إلى غربية ويزيل المسافات.
     * للاستخدام في WHERE عند البحث بهوية قد تكون مخزّنة بأرقام عربية من Excel.
     */
    public static function sqlNormalizeNationalIdExpr(string $column): string
    {
        $expr = $column;
        $map = [
            '٠' => '0', '١' => '1', '٢' => '2', '٣' => '3', '٤' => '4',
