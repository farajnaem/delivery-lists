...
| `APP_KEY` | نص عشوائي ثابت طويل (مثلاً 64 حرف) لتوقيع جلسات الأندرويد — **إلزامي** وإلا تنقطع الجلسة بعد كل Redeploy |
| `DATABASE_URL` | يُنسخ تلقائياً عند ربط MySQL |

> بعد أي تحديث لجلسة الموبايل: **Redeploy** من Coolify ثم ثبّت APK **1.0.4** جديد (النسخ القديمة ترسل `Authorization` وCoolify قد يعترضها).
